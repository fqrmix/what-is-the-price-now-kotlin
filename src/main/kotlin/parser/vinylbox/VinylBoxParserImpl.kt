...
class VinylBoxParserImpl(override val forceUpdate: Boolean = true) : Parser {
    ...
    private fun parseArticle(articleUrl: String) {
        println("Parsing page...")
        val document: Document = Jsoup.connect(URL(articleUrl).toString()).get()
        ...
        if (priceString != null) {
            ...
            article = Article(price, nameString, ShopName.VINYLBOX, articleUrl)
        }
    }
}