package org.example.parser.korobkavinila
...
/**
 * Реализация парсера для получения информации о товарах с сайта Korobka Vinila.
 *
 * @property forceUpdate Если `true`, каждый вызов метода [getArticleInfo] будет повторно загружать данные о товаре.
 */
class KorobkaVinilaParserImpl(override val forceUpdate: Boolean = true) : Parser {
    ...
    private fun parseArticle(articleUrl: URL) {
        try {
            ...
            val priceString = document.selectXpath(ParserConstants.KorobkaVinila.PRICE.value).text()
            val nameString = document.selectXpath(ParserConstants.KorobkaVinila.ARTICLE_NAME.value).text()
            val price = BigDecimal(
                priceString.filter { it.isDigit() }
            )
            article = Article(price, nameString, ShopName.KOROBKAVINILA, articleUrl.toString())
        } catch (e: Exception) {
